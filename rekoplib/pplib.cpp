...

{
	...
	forever {
		...
		if (eq) {
			...
			//20000814MB: After a few minutes of research, it seems that if

			// we want to send html formatted email, we could use 'mailto'.
			...
			// test: metasend -f /tmp/home.html -s "html test" -t "test@e-mediasoftware.com" -F "test@e-mediasoftware.com" -c "" -e 7bit -m text/richtext -b
