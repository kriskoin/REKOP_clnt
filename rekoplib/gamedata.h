...
// Data structure with affiliate information

struct Affiliate {
	char Name[MAX_COMMON_STRING_LEN] ;
	char Url[MAX_COMMON_STRING_LEN] ;
	char ID[MAX_COMMON_STRING_LEN] ;
	WORD16 Flags ;
};
...
/*
	...
	INT32  back_action[MAX_PLAYERS_PER_GAME];	
    INT32  no_limit_raise; 
*/
	...
	BYTE8  tournament_hand_number;		// game # for this tournament (1-255), 0 for non-tournaments
										...
										//20000921MB: gcd.tournament_hand_number is supported on the server but the client does not yet use it. Delete this comment when that changes.
...
#if 0	//20000918MB: not yet supported

#define GCDF_PLAYER_SITTING_OUT		0x40	// set if this player is being treated as if they're sitting out for this game (individualized for each client)
...
//	INT32  raise_amount;				// amount needed to raise   J Fonseca  01/02/2004
...
struct ClientPlatform {
	...
	//rgong 04/09/2002 for marketing purpose
...
struct CardRoom_TableSummaryInfo  {
	...
	BYTE8  tournament_hand_number;	// game # for this tournament (1-255), 0 for non-tournaments
									...
									//20000922MB: tsi.tournament_hand_number is supported on the server but the client does not yet use it. Delete this comment when that changes.

	WORD16 tournament_elapsed;	// # of seconds since tournament table started playing
...
#if 1	//20001003MB: shortened!

  #define SHOTCLOCK_MESSAGE_LEN	48
...
struct CardRoom_TableSummaryList  {
	...
	BYTE8  _unused_old_shotclock_msg[16];	//20001003MB: used to be shot clock message until it was shortened by 16 chars

	WORD16 active_tables;		// stat: # of currently active tables
...
struct CardRoom_ReqHandHistory {
	...
	//BYTE8  _unused[24];	// room for future expansion.
	BYTE8  playerName[24];	// room for future expansion.   //Cris 30/03/2004
};
...
// 19990812MB: a 500MB database file on CardRoom1 was scanned at a rate
...
#define MAX_PLAYER_SECURE_PHRASE_LEN		20  //Fonseca
...
typedef struct SDBRecord {	// database entry
	...
	char   external_id[MAX_PLAYER_USERID_LEN]; //for other systems like casinos
	...
	// Affiliate ID for first login - ricardoGANG - 8/22/2003 
	char   idAffiliate[MAX_COMMON_STRING_LEN];
	char   last_name[MAX_PLAYER_LASTNAME_LEN];
	// END Affiliate ID for first login - ricardoGANG - 8/22/2003 

/////////////////J. Fonseca  31/10/2003/////////////
...
/////////////////////////////////////////////////////
	...
	struct ClientPlatform client_platform;	// platform info from the last time they logged in
...
enum {
	...
	ACCOUNTRECORDUSAGE_KICKOFF_USERID,  //cris 30/03/2004

	ACCOUNTRECORDUSAGE_BLOCK_USERID, //cris 30/03/2004
...
#define MAX_PLAYERS_PER_CHECK_RUN		360	// 4 pages (20010102HK: changed from 180)
