...

{
   ...
   #if 0	//20000628MB: display winsock version info

	kp(("%s(%d) This Windows platform supports winsock version %d.%d to %d.%d\n",
...

{
	...
	zstruct(str);
		...
		if (result) {
		  ...
		  #if 0	//20001106MB: unfinished !!!!

			//20001106MB: cache any results for the .e-Media Poker.com domain.

			// We got a result, see if we need to write it to our cache file.

			#define DOMAIN_TO_CACHE		".e-mediasoftware.com"
			...
			//20001106MB: we did not find anything, try looking it up from the

			// hosts file in the current directory.
...

{
	...
	// 20000411MB: answer times up to 150s are not that uncommon for SFC
  ...
  #if 1	//20000412MB: sometimes there are really long delays

	#define AFTER_POST_TIMEOUT	600	// after writing, wait up to n additional seconds with no data before giving up.
	...
	if (sock != INVALID_SOCKET) {
		...
		shutdown(sock, 2);	//20000325MB: shut down sending and receiving on the socket
