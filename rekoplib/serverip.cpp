...
#endif
{
  ...
  #if 1	//20000327MB: I think this is what it should have been all along.
...
{
	if (listen_socket != INVALID_SOCKET) {
		shutdown(listen_socket, 2);	//20000325MB: shut down sending and receiving on the socket
...
ErrorType ServerListen::AcceptConnection(ServerSocket **output_server_socket)
{
	...
	*output_server_socket = NULL;	// initialize output to NULL.
	...
	if (new_socket == INVALID_SOCKET) {
		...
		// 20010312MB: after switching to quattro with the 2.2.18 kernel (from 2.2.14)
		...
		// 20010228MB: this problem also seems to occur occasionally after a lot of
	...
	*output_server_socket = new ServerSocket;
	...
	if (!*output_server_socket) {
		...
		shutdown(new_socket, 2);	//20000325MB: shut down sending and receiving on the socket
	...
	if (ssl_ctx) {
		...
		if (!ssl) {
			...
			LeaveSSLCritSec();
			//20000517MB: Note: we must still close the socket from here
			// because the ServerSocket object doesn't know about it yet.
			shutdown(new_socket, 2);	//20000325MB: shut down sending and receiving on the socket
			closesocket(new_socket);
		...
		//20010221mb: the SSL_accept() call seems to be taking about 60ms to
		// complete on a 700MHz P3 Xeon, even though it's supposed to be
