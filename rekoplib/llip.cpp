...
#endif
#define	VERSION_STRING				((const char *)"e-Media Poker IP lib 1.0")
...
ErrorType SocketTools::SetSocket(SOCKET new_socket)
{
	...
	if (oldsock != new_socket && oldsock != INVALID_SOCKET) {
	  #define TEST_CLOSING	0
		...
		int result;
		result = shutdown(oldsock, 2);	//20000325MB: shut down sending and receiving on the socket
		...
		close(s3);
	  #endif
         result=0 ; // == just to suppress warning in BCB 6
}
	return ERR_NONE;
}
...
ErrorType SocketTools::RawSendBytes(char *buffer, int len)
{
	...
	if (ssl) {	// SSL connection...
	  ...
	  #endif
	  #if INCLUDE_FUNCTION_TIMING && 0	//20000604MB
  ...
  #if 0	//20000201MB: commented out so we don't consider ourselves connected until we've read something
...
ErrorType SocketTools::ProcessSendQueue(void){
	...
	while (
	  ...
	  #if 0	//20010302MB: treat EVERYTHING as if it went through while tracking down memory leaks

		kp1(("%s(%d) *** WARNING: TREATING ALL PACKETS AS IF THEY GOT SENT!\n", _FL));
...

{
	...
	//19990517MB: strangely, the first few packets seem to get lost
	// sometimes for some unknown reason.  To avoid decompression problems,
	// we won't compress the first few packets.
   #if 1	//19990718MB: this may no longer be needed.  We shouldn't lose any initial packets.
