...
retry:	
		...
		if (kDebugOutputFileName) {
			...
			if (output_file != -1) {
			  ...
			  #if 0	//19991205MB: taken out because it can be very slow.

					// It seems that the data is at least given to the OS
...

{
 ...
 //DebugOutputFileName = "debwin.log";
	int pipes_allowed;	// set if we're allowed to test for \\.\pipe\*
		...
		//970320MB: if this is Win95, don't try to open pipes because they
...
//		DebugPipeType = 0;	// assume ANSI.SYS style output
	  ...
	  #if 0	//970411MB: I think this was only used for running under OS/2 with old debwin
...
//			{newms=GetTickCount();printf("%s(%d) elapsed_ms=%d\n",_FL,newms-oldms);oldms=newms;}
		...
		if (!DebugPipeHandle) {	// none of that worked, try a mail slot...
			...
			// 970715MB: it seems we can open a mailslot even if
