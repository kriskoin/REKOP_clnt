...
void SymFile_Load(void)
{
	  ...
	  #ifdef __WATCOMC__	//980115MB
		strnncpy(fname,_argv[0], MAX_FNAME_LEN);
	  #elif LINUX	// GCC
...
void DisplayStackCrawl(void)
{
	  ...
	  #if 0	//971118MB
		// Try to reverse engineer the format of the jmp_buf...
...
void DisplayStackCrawlEx(dword *stack_ptr, dword eip, char *thread_name)
{
		...
		// stop searching.
	  #if 1	//971229MB
		//kp(("%s(%d) searching a LOT of the stack\n",_FL));
	  ...
	  #if 0	//971118MB
		kp(("%s(%d) Functions are probably between $%08lx and %08lx\n",
		...
		while (stack_ptr < stack_top) {
			...
			if ((void *)*stack_ptr > code_start &&
			  ...
			  #if 0	//971118MB
				kp(("%s(%d)  possible return address: $%08lx\n", _FL, *stack_ptr));
